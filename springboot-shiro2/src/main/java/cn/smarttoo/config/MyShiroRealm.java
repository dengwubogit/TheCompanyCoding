...
import cn.smarttoo.pojo.SysPermission;
import cn.smarttoo.pojo.SysRole;
import cn.smarttoo.pojo.UserInfo;
import cn.smarttoo.service.UserInfoService;
...
public class MyShiroRealm extends AuthorizingRealm{
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		 ...
		 System.out.println("授权");
		  SimpleAuthorizationInfo authorizationInfo = new SimpleAuthorizationInfo();
	        // 取出realm域中的User对象
	        UserInfo userInfo = (UserInfo) principals.fromRealm(this.getName()).iterator().next();
	        // 
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		//调用业务判断用户是否存在
		UserInfo userInfo = userInfoService.findByUsername(upToken.getUsername());
		if(userInfo==null)
			return null;
		 //认证的返回值,只要返回值不为空就可以执行密码比较器---->执行此操作会自动将user对象放入realm域中
...
//        }
       for(SysRole role:userInfo.getRoleList()){
           ...
           for(SysPermission p:role.getPermissions()){
              authorizationInfo.addStringPermission(p.getPermission());
              System.out.println("xxxxxxx"+p.getPermission());
           }
       }
